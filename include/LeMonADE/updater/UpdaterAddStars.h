 ...
 *
 * @brief Updater to create a solution of monomdisperse stars.
 *
...
public:
  UpdaterAddStars(IngredientsType& ingredients_, uint32_t NChain_, uint32_t NMonoPerChain_, int32_t type1_=1, int32_t type2_=2, bool IsSolvent=false);
...
*/
template < class IngredientsType >
UpdaterAddStars<IngredientsType>::UpdaterAddStars(IngredientsType& ingredients_, uint32_t NChain_, uint32_t NMonoPerChain_, int32_t type1_, int32_t type2_, bool IsSolvent_):
