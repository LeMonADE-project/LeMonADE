 ...
 *
 * Includes tests for the following classes: Bondset
 * */
/************************************************************************/
...
class BondsetTest: public ::testing::Test{
public:
...
/************************************************************************/
//test the addBond methods of class Bondset
/************************************************************************/
TEST_F(BondsetTest, AddBonds){
  FastBondset bondset;
...
TEST_F(BondsetTest, CopyConstructor){
  ...
  //copy bondset
  FastBondset bondset_new(bondset);
...
/************************************************************************/
//test the getBondIdentifier and getBondVector methods of class Bondset
/************************************************************************/
TEST_F(BondsetTest,GetBondInformation){
	FastBondset bondset;
...
TEST_F(BondsetTest, LookupTable)
{
  ...
  VectorInt3 v6(4,0,0);
  VectorInt3 v7(1,-3,0);
  VectorInt3 v8(-6,0,0); //-> -6&7=2 ---> (2,0,0)


  EXPECT_THROW(bondset.addBond(5,1,2,100),std::runtime_error);
  EXPECT_THROW(bondset.addBond(1,1,5,100),std::runtime_error);
  EXPECT_THROW(bondset.addBond(1,5,2,100),std::runtime_error);
  ...
  EXPECT_FALSE(bondset.isValidStrongCheck(v1));

  EXPECT_FALSE(bondset.isValid(v2));
  EXPECT_FALSE(bondset.isValidStrongCheck(v2));

  EXPECT_FALSE(bondset.isValid(v3));
  EXPECT_FALSE(bondset.isValidStrongCheck(v3));

  EXPECT_FALSE(bondset.isValid(v4));
  EXPECT_FALSE(bondset.isValidStrongCheck(v4));

  EXPECT_FALSE(bondset.isValid(v5));
  EXPECT_FALSE(bondset.isValidStrongCheck(v5));

  EXPECT_FALSE(bondset.isValid(v6));

  EXPECT_FALSE(bondset.isValidStrongCheck(v6));

  EXPECT_FALSE(bondset.isValid(v7));
  EXPECT_FALSE(bondset.isValidStrongCheck(v7));

  EXPECT_FALSE(bondset.isValid(v8));
  EXPECT_FALSE(bondset.isValidStrongCheck(v8));
  ...
  // the simple check (isValid) gives the wrong answer - but this is expected.
  EXPECT_TRUE(bondset.isValid(v8)); //but it should be False
  // the Strong check (isValidStrongCheck) gives the right answer - but this is expected.
  EXPECT_FALSE(bondset.isValidStrongCheck(v8));
  // There is no problem about, because if have a 'right' start conformation
  // both are equal, only if you want to check for esp. unconnected monomers
